...
def compute_macro_for_multicite(preds, golds):
    # y_true_indices = [[0, 2], [1], [0, 2, 3]]
    # y_pred_indices = [[0], [1, 2], [0, 2]]

    # 2. Khởi tạo và khớp MultiLabelBinarizer
    mlb = MultiLabelBinarizer(classes=[0, 1, 2, 3, 4 , 5, 6]) 

    # Chuyển đổi sang ma trận nhị phân (0-1 matrix)
    y_true_bin = mlb.fit_transform(golds)
    y_pred_bin = mlb.fit_transform(preds)
    ...
    return {'macro-precision':macro_p , 'macro-recall':macro_r , 'macro_f1':macro_f1}
