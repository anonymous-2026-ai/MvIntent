...
class TripleEnhanceLayer(nn.Module):
	...
	def __init__(self,  word_emb_dim, n_iter , alpha,  device):
		...
		self.triple2intent = nn.ModuleList([CrossSigmoidBlock(self.word_emb_dim) for _ in range(self.n_iter)])
	...
	def forward(self, triple_embeddings , token_embedds, intent_embeds):
		triple_hid , token_hid , intent_hid = triple_embeddings , token_embedds, intent_embeds
		for i in range(self.n_iter):
			# token_hid = token_embedds
			triple_hid, intent_triple_sigmoid = self.intent2triple[i](intent_hid , triple_hid, alpha = self.alpha)
...
class CrossSigmoidBlock(nn.Module):
	...
	def forward(self, intent_embeddings , triple_embeddings, alpha):
		...
		return triple_embeddings + alpha * intent_information , torch.sigmoid(triple_intent_sigmoid).tolist() # 7, 768 
