...
class Model(nn.Module):
	...
	def __init__(self, pretrained_model, word_emb_dim, alpha , n_iter_word, device):
		super().__init__()
		self.device = device
		self.pretrained_model = pretrained_model
		self.word_emb_dim = word_emb_dim 
		self.alpha = alpha 
		print('Value of alpha ' , self.alpha)
		# self.linear_pool = nn.Linear(self.word_emb_dim , self.word_emb_dim , bias=True)
		self.triple_enhance = TripleEnhanceLayer(self.word_emb_dim  , n_iter_word, self.alpha , self.device)
		...
		self.intent_linears = nn.Linear(768 ,6 , bias=False)
	...
	def forward_pretrained_with_multilen_seq(self, list_seq , return_type = 'cls'):
		...
		# input_ids = pad_to_fixed_length(list_seq, max_len=510, pad_id=0)
		...
		if return_type == 'cls':
			...
			out = torch.mean(last_hidden_state[: , 1:-1 , :] , dim = 1) #(B,  768)
	...
	def forward(self, list_chunks_idx , list_triple_indexes ,  tokenizer):
		...
		for i in range(len(list_chunks_idx)):
			...
			if triple_indexes == []:
				list_batch_index.append([102] + chunk_idx  +  [103])
				list_len_chunk.append(1)
			else:
				list_len_chunk.append(len(triple_indexes))
				for j in range(len(triple_indexes)):
					list_batch_index.append([102] +  tokenizer.encode(triple_indexes[j] , add_special_tokens=False )  +  [103] )
		...
		for i in range(len(list_chunks_idx)):
			...
			word_level_index.append([102] + chunk_idx  +  [103])
		...
		for i in range(len(list_triple_embed)):
			triple_hids, intent_weight = self.triple_enhance(list_triple_embed[i] , list_tok_embed[i], intent_embeds )
...
def pad_to_fixed_length(list_seq, max_len, pad_id=0):
    """
    Hàm padding danh sách các chuỗi về một chiều dài cố định (max_len).
    
    Args:
        list_seq (list of list): Danh sách các chuỗi ID thô đầu vào.
        max_len (int): Chiều dài cố định mục tiêu (e.g., 256, 512).
        pad_id (int): ID của token [PAD], mặc định là 0.
        
    Returns:
        torch.Tensor: Tensor kết quả có kích thước cố định (B, max_len)
    """
    # 1. Chuyển list các chuỗi ID thành list các torch.Tensor (Giữ nguyên logic cũ của bạn)
    seqs = [torch.tensor(ids, dtype=torch.long) for ids in list_seq]
    
    # 2. Chạy pad_sequence mặc định để đưa về Lmax của batch hiện tại
    input_ids = pad_sequence(seqs, batch_first=True, padding_value=pad_id)  # Kích thước: (B, L_current_max)
    
    current_len = input_ids.shape[1]
    
    # 3. Điều chỉnh kích thước về đúng max_len mục tiêu
    if current_len < max_len:
        # Nếu ngắn hơn max_len -> Bù thêm các số pad_id vào phía bên phải (phía cuối chuỗi)
        pad_size = max_len - current_len
        input_ids = F.pad(input_ids, (0, pad_size), value=pad_id)
    elif current_len > max_len:
        # Nếu dài hơn max_len -> Cắt bớt (Truncate) lấy đúng khoảng chiều dài mục tiêu
