...
def load_triple(dataset):
	import json 
	path  = "data_acl_arc_triple/"+dataset+".json"
	out = []
	with open(path , 'r') as f_r:
		full_data = json.load(f_r)
		for data in full_data:
			out.append(data['list_triple_index_scibert'])
	return out 


def load_triple(args, dataset):
	import json 
	path  = args.data_dir + "/"+dataset+".json"
	out = []
	with open(path , 'r') as f_r:
		full_data = json.load(f_r)
		for data in full_data:
...
# def load_word_graph(args, dataset):
# 	import json 
# 	path  = args.data_dir+"/"+dataset+".json"
# 	out = []
# 	with open(path , 'r') as f_r:
...
def train(args, features, model, tokenizer, processor):
	...
	train_triple = load_triple(args, 'train')
	# train_word_graph = load_word_graph(args, 'train')
	...
	for epoch in range(int(args.num_train_epochs)):
		...
		for step, batch_index in enumerate(list_index_batch):
			...
			if args.classification_type == "multiclass":
				...
				# data_graph = [train_word_graph[t] for t in batch_index]
				...
				label = label_binary.argmax(dim=1).long().to(args.device)  # shape (B,) = (1,)
			...
			if args.logging_steps > 0 and global_step % args.logging_steps == 0:
				...
				with torch.no_grad():
					...
					results = evaluate(args, model, tokenizer, processor = processor , save_output = save_output, global_step= global_step)
...
def evaluate(args, model, tokenizer, prefix="", processor=None, save_output = '.',  global_step = 0 ):
	...
	for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
		...
		preds , out_label_ids = [] , [] 
		triple_test = load_triple(args, 'test')
		# word_graph_test = load_word_graph(args, 'test')		
		with open(save_output+'/triple_weight_'+str(global_step)+'.txt' ,'w') as f , open(save_output+'/triple_intent_att'+str(global_step)+'.txt' ,'w') as f1:
			import time 
			start = time.time()
			for  step,  batch_index  in enumerate(list_index_batch):
				model.eval()
				...
				with torch.no_grad():
					if args.classification_type == "multiclass":
						...
						# logits = model(chunks_idx , tok2sent , labels_idx , tok2label, tokenizer)
						...
						loss_func = CrossEntropyLoss()
						label_binary = torch.stack(label_binary , dim = 0)
						# label = torch.where(label_binary.type_as(logits) == 1)[0]
						label = label_binary.argmax(dim=1).long().to(args.device)  # shape (B,) = (1,)

						# print('labels ' , label_binary , logits)
						tmp_eval_loss = loss_func(logits, label)

					eval_loss += tmp_eval_loss.mean().item()
				nb_eval_steps += 1

				# preds.append(torch.squeeze(logits , dim = 0 ).detach().cpu().numpy().tolist())
			...
			# print('inference time ' , (end - start) / len(list_index_batch))
		...
		if args.classification_type == "multilabel":
			preds_old = preds
			pred_probs = torch.nn.functional.softmax(torch.tensor(preds_old), dim = 1).detach().cpu().numpy()
		...
		if args.do_eval and not args.do_test:
			...
			output_eval_file = os.path.join(save_output, prefix, "eval_results"+str(global_step)+".txt")
		else:
			# print('-------------------------------------')
			# print('vao ham nay 1 ...')
		...
		if not (args.task_name in ["pride"] and args.do_test):
			with open(output_eval_file, "w") as writer:
				logger.info("***** Eval results test {} *****".format(prefix))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
	...
	if output_mode == "multilabel_classification":
		# pad all label lists with -1 (-1 is not given to any class)
		# no! this was the old approach. Now, we use binary labels as it is easier to handle
		#max_num_labels = max([len(f.label) for f in features])
		# labels_binary = np.zeros((len(features), len(label_list)))
		# for i,f in enumerate(features):
		#     labels_binary[i][f.label] = 1
		# #[[f.label.append(-1) for i in range(max_num_labels - len(f.label))] for f in features]
		# all_labels = torch.tensor(labels_binary, dtype=torch.long)
		binary_labels = [] 
		for i,f in enumerate(features):
...
def main():
	...
	parser.add_argument("--save_model", default=0, type=int, help="Linear warmup over warmup_steps.")
	...
	model = Model(pretrained_model , 768 ,  args.alpha , args.n_iter_word, args.device)
	...
	# Training
	if args.do_train:
		features = load_and_cache_examples(args, args.task_name, tokenizer, evaluate=False, test=False)
		global_step, tr_loss = train(args, features, model, tokenizer, processor = processor)
	...
	if args.do_eval or args.do_test and args.local_rank in [-1, 0]:
		if args.do_test and not args.do_eval:
		...
		for checkpoint in checkpoints:
			...
			model.to(args.device)
			result = evaluate(args, model, tokenizer, prefix=prefix, processor=processor)
			result = result[0]
